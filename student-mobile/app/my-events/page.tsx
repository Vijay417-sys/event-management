        ...
        <div className="relative z-10 container mx-auto px-4 pt-8">
          <div className="text-center mb-8 animate-slide-up-mobile">
            <div className="inline-block p-4 glass-mobile mb-4">
              <h1 className="text-4xl font-bold text-white mb-2 drop-shadow-lg">📋 My Events</h1>
            </div>
            <p className="text-lg text-white/90 font-medium drop-shadow-md max-w-sm mx-auto">Events you have registered for</p>
          </div>
          
          <div className="space-y-6">
            {events.map((event, index) => (
              <div key={event.event_id} className="card-mobile-interactive animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                ...
                <div className="flex space-x-3">
                  <button onClick={() => openFeedbackForm(event)} className="btn-mobile flex-1 text-center group">
          ...
          {events.length === 0 && (
            <div className="text-center py-16 animate-fade-in">
              ...
              <a href="/events" className="btn-mobile inline-block">Browse Available Events</a>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in">
            <div className="bg-white rounded-3xl p-6 w-full max-w-md animate-slide-up-mobile">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={closeFeedbackForm} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200 transition-colors">✕</button>
              </div>
              ...
              <form onSubmit={handleFeedbackSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="flex space-x-2">
                    {[1,2,3,4,5].map((star)=>(
                      <button key={star} type="button" onClick={()=>handleRatingClick(star)} className={`w-10 h-10 rounded-full flex items-center justify-center text-xl transition-all duration-200 ${ star <= feedbackForm.rating ? 'bg-yellow-400 text-white scale-110' : 'bg-gray-200 text-gray-400 hover:bg-gray-300' }`}>⭐</button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea value={feedbackForm.feedbackText} onChange={(e)=>setFeedbackForm(prev=>({...prev, feedbackText: e.target.value}))} className="input-mobile w-full h-24 resize-none" placeholder="Share your experience..." />
                </div>
                
                {feedbackMessage && (
                  <div className={`p-3 rounded-xl text-center font-medium ${feedbackMessage.includes('✅') ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
                    {feedbackMessage}
                  </div>
                )}
                
                <div className="flex space-x-3">
                  <button type="button" onClick={closeFeedbackForm} className="btn-mobile-secondary flex-1">Cancel</button>
                  <button type="submit" className="btn-mobile flex-1" disabled={feedbackForm.rating === 0}>Submit Feedback</button>
                </div>
              </form>
            </div>
          </div>
