...
def init_db():
    ...
    print("✅ MySQL connected & tables initialized")
...
@app.route("/events", methods=["GET"])
def get_events():
    return jsonify(execute_query("SELECT * FROM Events ORDER BY date", fetch=True))
...
def create_event():
    ...
    event_id = execute_query(
        "INSERT INTO Events (college_id,name,type,date) VALUES (%s,%s,%s,%s)",
        (data["college_id"], data["name"], data["type"], data["date"])
    )
...
def find_or_create_student():
    ...
    student_id = execute_query(
        "INSERT INTO Students (college_id,name,email) VALUES (%s,%s,%s)",
        (data["college_id"], data["name"], data["email"])
    )
...
def mark_attendance():
    ...
    execute_query(
        ...
        """,
        (data["student_id"], data["event_id"], data["status"], data["status"])
    )
    return jsonify({"message": "Attendance marked"})


@app.route("/staff/attendance/<int:event_id>")
...
def staff_events():
    return jsonify(execute_query("""
        SELECT e.*, COUNT(r.registration_id) registration_count
        FROM Events e
        LEFT JOIN Registrations r ON e.event_id=r.event_id
        GROUP BY e.event_id
    """, fetch=True))
...
def staff_feedback():
    return jsonify(execute_query("""
        SELECT f.*, s.name student_name, s.email
        FROM Feedback f
        JOIN Students s ON f.student_id=s.student_id
    """, fetch=True))
